...
module OpenSocial #:nodoc:
  ...
  # Provides the ability to request a Collection of profileComments for a given
  ...
  # a user ID and selector (and optionally an ID of a particular profileComment).
  ...
  class FetchProfileCommentsRequest < Request
	...
	# Initializes a request to fetch  profileComments for the specified user and
  ...
  class UpdateProfileCommentsRequest < Request
	...
	# Initializes a request to update  profileComments for the specified user and
    # group, or the default (@me, @self). A valid Connection is not necessary
    # if the request is to be used as part of an RpcRequest.
    def initialize(connection = nil, guid = '@me', selector = '@self',
                   pid = nil)
