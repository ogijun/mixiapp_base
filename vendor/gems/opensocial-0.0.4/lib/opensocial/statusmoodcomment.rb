...
module OpenSocial #:nodoc:
  ...
  # Provides the ability to request a Collection of statusMoodComments for a given
  # user or set of users.
  #
  # The FetchStatusMoodRequest wraps a simple request to an OpenSocial
  # endpoint for a Collection of statusMoodComments. As parameters, it accepts
  # a user ID and selector (and optionally an ID of a particular statusMoodComments).
  ...
  class FetchStatusMoodCommentsRequest < Request
	...
	# Initializes a request to fetch statusMoodComments for the specified user and
  ...
  class UpdateStatusMoodCommentsRequest < Request
	...
	# Initializes a request to update statusMoodComment for the specified user and
    # group, or the default (@me, @self). A valid Connection is not necessary
    # if the request is to be used as part of an RpcRequest.
    def initialize(connection = nil, guid = '@me', selector = '@self',
                   pid = nil)
